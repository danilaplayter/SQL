...
SELECT users.name, user_spending.total
FROM user_spending
JOIN users ON users.id = user_spending.user_id
WHERE total > (SELECT AVG(total) FROM user_spending);
...
FROM (
    SELECT
        ...
        RANK() OVER (PARTITION BY products.category ORDER BY SUM(order_items.quantity) DESC) as rank
    FROM products
